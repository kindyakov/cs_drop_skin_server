...
```
GET   /api/v1/user/:id            → Профиль пользователя по ID (публичный, опциональный JWT)
...
- Если авторизован И запрашивает свой профиль → возвращает расширенные данные (balance, tradeUrl, isBlocked)
- Если не авторизован ИЛИ чужой профиль → возвращает публичные данные
