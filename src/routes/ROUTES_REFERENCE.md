...
POST  /api/v1/user/items/sell-all    → Продать все предметы со статусом OWNED (JWT)
```
...
**Массовая продажа всех предметов (POST /api/v1/user/items/sell-all):**

Позволяет пользователю продать все предметы со статусом `OWNED` одним запросом.

**Требования:**

- JWT авторизация
- Хотя бы один предмет со статусом `OWNED` в инвентаре

**Request:**

```bash
POST /api/v1/user/items/sell-all
Authorization: Bearer <JWT_TOKEN>
```
...
{
  "success": true,
  "message": "Успешно продано предметов: 15 на сумму 5400.00₽",
  "data": {
    "totalSold": 15,
    "totalAmount": 540000,
    "newBalance": 690000,
...
- `totalSold` - количество проданных предметов
- `totalAmount` - общая сумма продажи (n% от цен всех предметов, в копейках)
- `newBalance` - новый баланс пользователя после продажи (в копейках)
- `itemNames` - массив названий всех проданных предметов
...
{
  "success": false,
  "message": "У вас нет предметов для продажи. Продать можно только предметы со статусом \"OWNED\""
}
```

**Бизнес-логика:**

- Комиссия сайта: 20% (как при одиночной продаже)
- Операция атомарная (транзакция): статус всех предметов меняется на `SOLD` + баланс увеличивается
- Все операции логируются для аудита
- Продаются только предметы со статусом `OWNED` (уже проданные или выведенные игнорируются)
