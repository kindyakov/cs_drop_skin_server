...
POST  /api/v1/user/item/:id/sell   → Продать скин за n% от цены (JWT)
```
...
**Продажа скина (POST /api/v1/user/item/:id/sell):**

Позволяет пользователю продать скин обратно сайту за n% от рыночной цены.
...
```bash
POST /api/v1/user/item/cmh412pc90000iiy0ovjcym6g/sell
...
- `soldPrice` - сумма, полученная пользователем (n% от цены, в копейках)
...
- `item.sellPrice` - цена продажи с учетом комиссии (n% от рыночной цены, в копейках)
